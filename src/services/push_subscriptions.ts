...
const getPushSubscriptions = async (): Promise<PushSubscription[]> => {
  const { data } = await axios.get(`/api/push_subscriptions`);
  return data;
};

const usePushSubscriptions = (): UseQueryResult<PushSubscription[], Error> => {
...
export { usePushSubscriptions, useUpsertPushSubscription };